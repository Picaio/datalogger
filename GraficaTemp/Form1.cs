...
namespace GraficaTemp
{
    public partial class Temperatura : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            dlg.DefaultExt = ".cvs";
            dlg.FileName = "Datos_originales";
            dlg.Filter = "Fichero CSV(*.csv)| *.csv";
            DialogResult dlgResult = dlg.ShowDialog();
            if (dlgResult == DialogResult.OK)
            {
                StringBuilder csvMemoria = new StringBuilder();
                csvMemoria.AppendLine("`Dato 1;Dato 2; Tiempo");
                ...
                for (int j = 0; j <= listBox1.Items.Count - 1; j++)
                {
                    ...
                    {
                        csvMemoria.AppendLine("" + Convert.ToDouble(arreglo[0]).ToString().Replace(".", ",") + ";" + Convert.ToDouble(arreglo[1]).ToString().Replace(".", ",")+";" + Convert.ToDouble(arreglo[2]).ToString().Replace(".", ","));
                    }
                }
                ...
                sw.Close();
                MessageBox.Show( "Datos guardados en CVS", "CVS");
